...
import { Button, TabStrip, type TabDef } from '@/shared/ui';
...
import { EnvironmentSwitcher } from '@/modules/environments';
import { LanguageSwitcher } from '@/modules/language';
import { PageShortcuts, FavoritesView } from '@/modules/pages';
...
export default function MainView({
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      // Ctrl+B: favourite current page — works even inside inputs (if bookmarks enabled)
      if (bookmarksEnabled && e.ctrlKey && !e.metaKey && !e.altKey && e.key === 'b') {
        e.preventDefault();
        onFavoriteCurrentPage();
        return;
      }
      ...
      switch (e.key) {
        case '/':
          e.preventDefault();
          if (!bookmarksEnabled || activeTab === 'envs') setFocusEnvSearch(n => n + 1);
  ...
  }, [activeTab, bookmarksEnabled, isConfigured, navigate, onFavoriteCurrentPage, showHelp]);
  ...
  if (isConfigured) {
    return (
      <div className="relative flex flex-col h-full overflow-hidden">
            ...
            <div className="px-5 py-4" onClick={e => e.stopPropagation()}>
              ...
              {([
                ...
                },
                {
                  label: 'Search',
                  rows: [
                    { keys: ['/'], desc: bookmarksEnabled ? 'Focus search in current tab' : 'Focus environment search' },
        ...
        {bookmarksEnabled && currentTab?.url && currentTab?.currentEnvironment && (
          <button
